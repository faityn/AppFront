...
const Page = () => {
  ...
  const getData = async () => {
    const response = await getProductsList(1);
  ...
  return (
    <Layout type="main">
      ...
      <div className="mx-auto max-w-[1224px] mt-[60px] mb-[150px] whitespace-pre-line break-keep relative">
        ...
        <div className={`${router.query.id ? "hidden" : ""} text-black`}>
                        ...
                        <Link href={`/products?id=${item?.id}`}>
                          <div className="">
                            <Image
                              ...
                              alt={``}
                              className=" w-full min-h-[200px] max-h-[200px] max-sm:max-h-[304px] object-cover object-center"
                              width="392"
                              height="200"
                              unoptimized={true}
                              priority={true}
                          ...
                          <div className="text-xl max-sm:text-md mt-[10px] text-[#353644]">
                            {locale === "kr" ? item?.title : item?.title_en}
                          </div>
                        </Link>
                      </div>
                    </div>
          ...
          <div className={`${activeTab === "tab4" ? "" : "hidden"} mt-20`}>
            <div className="grid grid-cols-12 gap-5 max-md:px-4">
              {itemsList?.map((item, index) => {
                if (item?.sub_category === 4) {
                  return (
                    <div
                      key={index}
                      className="col-span-3 max-md:col-span-6 max-sm:col-span-12"
            ...
            <div className="">
              ...
              {locale === "kr" ? (
                ...
                <div
                  className="text-[#555555]"
                  dangerouslySetInnerHTML={{
                    __html: itemsDetail[0]?.content_en as string,
                  }}
                ></div>
              )}
            </div>
